...
最后更新：2026-07-15（**清理内存诊断设施（删除 dead code）**：内存诊断任务已完成（根因定位为 ort CPU arena 首次大图 OCR 一次性 jump 后不回落，已用空闲回收方案解决），诊断设施是临时观测点，全部删除。删除内容：① `src-tauri/src/diag.rs`（进程树内存采样模块，整文件删）；② `scripts/mem-diag.ps1`（压测脚本，整文件删）；③ `crates/vendor/oar-ocr-core/`（关 arena 的 vendor patch，整目录删——经查从未生效，paddleocr.rs 没设 execution_providers 导致 patch 代码被跳过；且最终用空闲回收方案，不需要 vendor patch）+ `Cargo.toml` 的 `[patch.crates-io]` 段（oar-ocr-core 恢复用上游 0.7.1）；④ `src-tauri/Cargo.toml` 删 `windows` 依赖（仅 diag.rs 用）；⑤ 删 `diag_run_ocr_pipeline`（ocr_translate.rs，~130 行诊断管线）+ main.rs 的 `--diag-ocr` argv 处理 / `parse_diag_bbox` / `mod diag`；⑥ 删 `log_diag` 命令（capture.rs）+ invoke_handler 注册 + api.ts 的 `logDiag` 封装；⑦ 删 capture/recognize/translate/history 各处的 `[mem_diag]` 打点 + 前端 Capture.vue `capture_timing` 打点 / Result.vue `result_ocr_done`+`result_translate_done` 打点 / main.ts 的 ui_error 全局错误转发；⑧ 删 state.rs 的 `diag_lock`（诊断管线单飞锁，无消费方了）。`check_file` 命令保留（通用文件检查，非诊断专供）。**验证**：fmt + clippy -D warnings（0 警告）+ test（src-tauri 19 全过）+ vue-tsc（0 错误）。）（**OCR session 空闲回收（释放 ort arena）**：诊断数据证伪了"渐进泄漏"假说——日志显示是**首次全屏 OCR 一次性 jump 到 main_priv_mb=1806 后保持高位**（后续稳定 1806-1837 不涨），WebView2 子进程稳定 ~250MB。根因 ort CPU BFCArena 只随 session drop 归还 OS（rc.12 无"推理后释放"API），首次大图推理 arena 按最大输入 shape 攒住 ~1G+。**重要发现**：项目 `crates/vendor/oar-ocr-core/` 的关 arena patch（`ort_infer_config.rs` 的 `with_arena_allocator(false)`）**从未生效**——`paddleocr.rs` 构造 `OrtSessionConfig` 时没设 `execution_providers`，字段为 None，`apply_ort_config` 里 `build_execution_providers` 整段被跳过，session 走默认 CPU EP arena 仍开。用户选择不修接线（怕推理变慢 10-30%），改用**空闲 10 分钟回收 session** 策略——平时保留 session 保响应速度，空闲超时 drop 释放内存，下次 OCR 懒重建。**GPT 评审修正 3 个 High 风险**：① 原 `ocr: Mutex<Option<Arc>>` + 独立 `last_ocr_at` 两把锁有 TOCTOU 竞态（tick 读超时放锁→等 ocr 锁期间用户正好 OCR/重建→tick 把新 provider 清空=抖动）→ 合并为 `Mutex<OcrSlot{provider, last_used}>`，回收判定与清空在同一次持锁内。② 并发懒重建惊群（多请求见 None 各自加载一套 ONNX session）→ 加 `ocr_init: Mutex<()>` 单飞锁 + 二次检查。③ `PaddleOcrProvider::new` 是同步重活别阻塞 tokio worker → `spawn_blocking`。另加 `Arc::strong_count==1` 检查避免回收正在推理的 session（否则 clone 的 Arc 还活着=arena 不释放+下次又重建=重复加载）。**4 处改动**：① `state.rs`：`ocr` 字段改 `Mutex<OcrSlot>`（合并 provider+last_used 消除竞态）+ 新增 `ocr_init: Mutex<()>` 单飞锁。② `config_cmd.rs`：抽 `build_ocr_provider(tier)` 纯构造 + `ensure_ocr_provider(app)` 懒确保存在（单飞+spawn_blocking+刷新 last_used）+ `reload_ocr_provider` 命令改强制重建（无条件替换，下载/切档位用）。③ `ocr_translate.rs`：`recognize_region` 加 `app: AppHandle` 参数，OCR 获取走 `ensure_ocr_provider`。④ `main.rs`：`setup` 末尾 `spawn_ocr_idle_reclaimer` 后台 tick——每 60s 检查，`strong_count==1 && elapsed>=OCR_IDLE_TIMEOUT(10min)` 才置 None，打 `idle_secs` 日志。**验证**：fmt + clippy -D warnings（0 警告）+ test（src-tauri 24 全过）。**手动验证留给用户**：把 `OCR_IDLE_TIMEOUT` 临时改 30s，跑一次全屏 OCR（任务管理器看内存 jump 到 ~1.8G），停手等 2 个 tick（~2 分钟）看日志出现"OCR session 空闲超时，已回收"，确认任务管理器内存回落到启动基线附近，再 OCR 一次确认懒重建只触发一次且内存重新可回收。GPT 还指出 Onboarding/Settings 吞 reload 错误仍提示"下载完成"是现有 bug，**本次不处理**（精准修改）。Grok 评审失败（provider server error），由 GLM 自行补充核对兜底。改动文件：`src-tauri/src/{main,state}.rs`、`src-tauri/src/commands/{config_cmd,ocr_translate}.rs`、`docs/CODE_MAP.md`。）（**诊断工具链修复（GPT+Grok 双模型评审 + GLM 核验纠错）**：用户实测 `mem-diag.ps1` 输出全 `?`，且 30 轮 `ocr_after` 仅出现 3 次。两个根因——① **脚本解析全失效**：file_layer 默认 `with_ansi(true)` 把 ANSI 转义码写进了 `snaptext.log`，且 tracing 结构化字段值带引号，脚本正则在匹配阶段就全灭。② **ocr_after 大量缺失**：single-instance 回调每次 `spawn` 新诊断任务（`main.rs:78`），脚本又固定 5s 间隔 fire-and-forget，前一轮 ORT 推理未完成就叠加，并发推理污染 arena 曲线（ORT 日志 `Total allocated bytes 286→554→1091MB` 印证）。**评审纠错**：原归因"持 AppState OCR 锁"被 Grok 用代码反证——`state.ocr.lock().await.clone()` 只克隆 provider，锁不跨 `run_ocr` 持有，并发任务真正并行进推理。**4 处改动**：① `main.rs:263` file_layer 加 `.with_ansi(false)`（per-layer，GPT+Grok 确认只影响文件层，stderr 仍带色）。② `state.rs` AppState 加 `diag_lock: tokio::sync::Mutex<()>`（单飞锁）。③ `ocr_translate.rs:117` `diag_run_ocr_pipeline` 入口 `try_lock`——抢不到锁说明上一轮在跑，打 `node="diag_busy"` 并返回 Err（`_diag_guard` 持有到函数结束释放）。④ `mem-diag.ps1` 重写：`Strip-Ansi`（整行剥 `\x1b\[...m`，单引号 pattern 兼容 PS 5.1）+ `Parse-MemDiagFields`（键名正则逐键抽，替代 split——extra 字段值含空格会碎裂）+ `Wait-Iteration`（每轮记起点行号，**增量窗口**轮询等终态——`诊断管线完成`/`诊断 OCR 管线失败`/`diag_busy`/crash，超时则**停止整个压测**防堆积）+ baseline 改用第一轮 ocr_before（旧版 `$startLogLine` 在循环前设末尾→baseline 永远 null）。**不引入 run_id 协议**——串行化后并发消失，本轮窗口内 ocr_before→ocr_after 行号配对天然不交叉。**验证**：fmt + clippy -D warnings（0 警告）+ test（src-tauri 24 全过）。改动文件：`src-tauri/src/{main,state}.rs`、`src-tauri/src/commands/ocr_translate.rs`、`scripts/mem-diag.ps1`。）（**内存诊断门禁（不改业务逻辑，纯加观测点）**：用户反馈"内存一开始 150M，用久了涨到 1.5G 左右"。这是**渐进增长**，与 2026-07-02 修过的"单次大图峰值不回落"是不同症状。初步根因推断是 ort CPU arena 渐进扩容，但经 GPT+Grok 双模型评审，**一致认为在未用分进程 RSS 曲线证实主因归属前，不应落地"定期重建 OCR session"等改动**（违反简洁优先，且有并发风险）。本次只加诊断能力，收集数据后再针对性修复。**新增 `src-tauri/src/diag.rs`**：Windows API（`GetProcessMemoryInfo` + `CreateToolhelp32Snapshot`）采样进程树内存——主进程（Rust，含 ort arena/堆）vs 子进程树（WebView2 renderer/GPU/network）。取 `WorkingSetSize`（工作集）和 `PrivateUsage`（私有字节，判断泄漏首选）。**打点位置**：后端 4 节点（capture/recognize/translate/history_list+search 前后，输出到 tracing 日志 target=`mem_diag`）+ 前端 3 节点（Capture.vue hide / Result.vue OCR done / translate done，经 `log_diag` 回传）。`src-tauri/Cargo.toml` 加 `windows` 依赖（0.62，与 core 同版本，features: `Win32_System_ProcessStatus`/`Win32_System_Threading`/`Win32_System_Diagnostics_ToolHelp`）。**验证**：fmt + clippy -D warnings（0 警告）+ test（core 46 + src-tauri 19 全过）+ vue-tsc（0 错误）。改动文件：`src-tauri/src/{diag.rs(新),main.rs,commands/{capture,ocr_translate,history}.rs}`、`src-tauri/Cargo.toml`、`src/views/{Capture,Result}.vue`。**用户实测步骤**：跑复现场景后看 `%APPDATA%\SnapText\logs\snaptext.log` 里 `[mem_diag]` 时间线——场景A固定区域框选50次(若 main_priv 线性涨=ort arena) / 场景B不同尺寸框选(验证动态shape扩容) / 场景C关auto_ocr只截图(看children_ws是否仍涨=WebView2) / 场景D反复打开历史面板(验证screenshot_png BLOB全量读取)。数据出来再决定下一轮改哪里。）（**补充自动化测试**：① diag.rs 加 5 个 `#[cfg(test)]` 单测——`snapshot_does_not_panic`（采样调用链安全）、`main_private_bytes_positive`（主进程私有字节>0证明采样有效）、`main_working_set_ge_private`（工作集>=私有字节的 OS 语义不变式）、`mb_conversions_are_safe`（MB 换算边界）、`collect_descendants_finds_spawned_child`（spawn 真实子进程验证进程树 BFS 枚举正确）。纯 Rust 单测，CI 可跑，不依赖桌面/模型。② 新增 `scripts/mem-diag.ps1`——驱动连续框选 + 从日志尾部解析 `[mem_diag]` 的 `main_priv_mb`/`children_priv_mb` + 输出 CSV 报告 + 判读指南。支持 `-Scenario fixed/random`（固定坐标 vs 随机坐标对比）。**编码陷阱**：PowerShell 5.1 解析含中文的 .ps1 脚本需要 **UTF-8 with BOM + CRLF**（LF-only 无 BOM 会因 here-string 边界错乱报语法错误），脚本已按此编码保存。**验证**：cargo test -p snaptext diag（5 通过）+ fmt + clippy -D warnings（0 警告）+ PS 语法检查 + 空跑验证。改动文件：`src-tauri/src/diag.rs`、`scripts/mem-diag.ps1`。）（**补充：诊断管线 argv 触发（绕过 GUI/鼠标）**：用户反馈 mem-diag.ps1 的鼠标模拟框选不生效（热键能触发选区窗，但 `mouse_event` 模拟的拖拽丢失——选区窗 show 有异步时序：截图→前端拉 BMP→onload→双层 rAF→show，总 ~300-600ms 浮动，脚本发鼠标事件时窗口可能还没准备好接收）。**方案**：加后端诊断入口，脚本经 `single-instance` argv 触发，在 Rust 内跑完整管线，完全绕过选区窗口和鼠标。① `ocr_translate.rs` 新增 `diag_run_ocr_pipeline(app, bbox)`（**非 `#[tauri::command]`**，内部函数）——串接 do_capture_all→crop_frame→run_ocr→run_translate，复用核心纯函数 + 既有 `[mem_diag]` 打点；`bbox=None` 全屏、`Some` 指定尺寸（random 场景）；诊断不写 PNG 文件（省 IO）、不依赖前端 auto_ocr 开关。② `main.rs` single-instance 回调识别 argv `--diag-ocr`[=`WxH`]，`parse_diag_bbox` helper 解析可选尺寸（坐标固定 0,0，诊断只关心尺寸测动态 shape）；复用热键回调同款 `app_handle.clone()` + `tauri::async_runtime::spawn` 模式。③ `mem-diag.ps1` 重写——去掉全部 Win32 `mouse_event`/`keybd_event`/`Add-Type` 代码，改为 `Start-Process snaptext.exe --diag-ocr`（single-instance 保证不启动新实例，argv 送达已运行实例）；自动定位 exe（优先 `%LOCALAPPDATA%\SnapText\`，回退 `target\{release,debug}\`）；`-Scenario` 改为 `full/random`（全屏 vs 随机尺寸）。**踩坑**：PowerShell 5.1 含中文脚本须 UTF-8 with BOM + CRLF，且**不能有重复 BOM**（重复 BOM 会让 param 块解析崩溃报"赋值表达式无效"，极难定位——最终用字节级检查发现三重 BOM）。**验证**：fmt + clippy -D warnings（0 警告）+ test（core 46 + src-tauri 24 全过）+ PS 空跑（正确定位 exe + 检测 SnapText 未运行报错）。改动文件：`src-tauri/src/commands/ocr_translate.rs`、`src-tauri/src/main.rs`、`scripts/mem-diag.ps1`。**手动验证留给用户**：SnapText 运行时跑 `.\scripts\mem-diag.ps1` → 日志应出现完整 capture/ocr/translate 的 `[mem_diag]` 时间线 + CSV 报告。）

2026-07-15（**修复模型缺失未跳引导页 + 设置页切档位不显示下载按钮**：用户反馈"已完成引导但模型被删/损坏后进软件不跳引导页""设置页切换 OCR 档位后没显示下载按钮"。根因两处独立。① **Home.vue 模型缺失不跳引导页**：`onMounted` 只判断 `onboarding_completed===false` 才跳引导页，已完成引导的用户模型缺失时 `onboarding_completed` 已 true，停留在首页只显示"模型：缺失"状态卡却无下载入口。**修复**：查完 `modelsReady` 后，模型缺失也 `router.replace('/onboarding')`。引导页本身有 `is_models_ready()` 幂等检查（已就绪显示"✓ 无需下载"），不会重复下载。② **Settings.vue 切档位不显示下载按钮**：tier `n-select` 漏绑 `@update:value`，`checkModels()` 只在 `onMounted` 调一次，切档位后 `modelsReady` 仍为旧档位值——若旧档位已下载、新档位未下载，切换后仍显示"✓ 模型已就绪"，下载按钮不出现。**修复**：tier `n-select` 加 `@update:value="checkModels"`，切换后重新检查该档位下载态刷新下载按钮显示（无需立即 save，`downloadModels` 内部下载前已落盘 tier，与现有设计一致）。**引导页起点**：模型缺失跳引导页时仍从第一步（快捷键）开始——用户已配过的快捷键会回显，多点一次"下一步"到第二步下载，改动最小不碰 Onboarding.vue onMounted。**后端零改动**。**验证**：vue-tsc 0 错误。改动文件：`src/views/{Home,Settings}.vue`。手动验证留给用户：① 删 models 目录 + onboarding_completed=true → 进软件应跳引导页；② 引导页下载完模型 → 回首页正常；③ 设置页切 medium→small（若 small 未下载）应出现下载按钮。）

2026-07-14（**修复设置页/历史页窗口无 Tauri 权限**：用户反馈"设置页下载模型下不动，但指引页正常"。根因：`capabilities/default.json` 的 `windows` 白名单只有 `["main","capture","result"]`，**漏列 `settings` 和 `history` 两个独立窗口 label**。设置/历史窗口由 `window.rs::open_panel` 用 `WebviewWindowBuilder` 创建（label 分别为 `"settings"`/`"history"`），不在权限白名单内→该窗口拿到的是空权限集→所有 `invoke` 命令调用被 Tauri 拒绝、`listen` 事件监听不生效，页面纯前端部分能渲染但后端交互全静默失败，表现为"下载按钮点了没反应/下不动"。指引页 `Onboarding.vue` 跑在 `main` 窗口内的路由（`#/onboarding`，由 `Home.vue` `router.replace` 跳转），`main` 在白名单内故一切正常——这是两页表现不一致的唯一原因。**修复**：`windows` 数组补 `"settings"`、`"history"`（一行 JSON 改动，零行为风险）。**为什么设置页其他功能（保存配置等）看似能用**：实际上也调不通，只是用户没触发到报错路径；补权限后所有设置页命令一并修复。改动文件：`src-tauri/capabilities/default.json`、`docs/CODE_MAP.md`。**验证**：vue-tsc 不受影响（纯后端配置）；用户手动验证设置页下载模型可用 + 历史窗口列表/搜索可用。）（**修复结果窗口复用时不刷新 bug**：用户反馈"快捷键截图→结果窗口→再按快捷键框选，结果窗口停在第一次的内容没刷新"。根因：`Capture.vue::onUp` 用固定 label `"result"` 调 `new WebviewWindow`，第二次框选时 Tauri 发现同 label 窗口已存在→不重建页面→`Result.vue::onMounted` 不会再执行→不会重新拉 `last_crop`/OCR/翻译，后端 `state.last_crop` 已是新数据但前端不取。**修复**：结果窗口改"存在则复用刷新、不存在才新建"。① `Capture.vue::onUp` crop 后 `WebviewWindow.getByLabel("result")` 判断：存在则 `emit("result-refresh")` + `show()` + `setFocus()`，不存在则 `new`（首次与手动关闭后重建）。② `Result.vue` 把 `onMounted` 流程抽成 `refresh()` 函数，`onMounted` 注册 `listen("result-refresh", refresh)` + 首次调一次，`onBeforeUnmount` 清理 unlisten。③ refresh 完整重置 16 个响应式字段（漏一个会残留旧内容）+ 重设 `t0Base` 耗时基准。④ **generation 守卫**：`runOcr`/`runTranslate` 接 `gen` 参数，每个 `await` 后比对 `gen !== generation` 丢弃 stale 结果——medium 档 OCR ~3s，用户可能在第一次未跑完就框选第二次，旧请求后完成时丢弃不更新 UI、不弹错误 toast。⑤ **`img.src` 加 `?t=Date.now()` 防缓存**（从"次要"升级为"必须"）：`write_crop_png` 文件名按 monitor_id 固定（`result_<id>.png`），复用窗口时 URL 与上一次完全相同→浏览器不触发 onload→canvas 底图不更新；与 `Capture.vue` 选区窗 `?t=` 防缓存对齐。⑥ refresh 失败不 close 窗口（保留让下次快捷键自动修复），与第一次失败行为统一。**边界已处理**：OCR/翻译进行中触发第二次框选（generation 守卫）、用户关了结果窗口后框选（getByLabel 返回 null 走 new）、窗口最小化时收到 refresh（show+setFocus 唤起）、手动模式下按钮 disabled 防重复触发。**后端零改动**（`state.last_crop`/`last_ocr` 接力缓存已支持反复写入）。**验证**：vue-tsc 0 错误 + clippy -D warnings 0 警告（后端未动）。改动文件：`src/views/{Capture,Result}.vue`。手动验证留给用户：第一次快捷键出结果窗口→不关→第二次快捷键框选→结果窗口内容应更新为新截图/译文。）
...
|---|---|---|
| `mod.rs` | 模块导出 | — |
| `config_cmd.rs` | `get_config` / `save_config`（写盘+重建翻译 Provider+重注册热键，结果写回 `hotkey_error`）/ `check_translate_ready` / `get_default_prompt`（返回 `prompt.rs::DEFAULT_PROMPT_TEMPLATE` 供前端只读展示，单一数据源）/ `get_hotkey_status`（返回热键注册状态 `Option<String>`，None=成功/Some=失败原因，前端用于提示）/ `complete_onboarding`（置 `config.general.onboarding_completed=true` 并落盘，不复用 `save_config`——只置标志，不重建 Provider/不重注册热键）/ `reload_ocr_provider`（**强制重建**——无条件按当前 tier 重建 OCR Provider 写回 `state.ocr`，模型下载完成/切档位后调，与 `ensure_ocr_provider` 共用 `build_ocr_provider` 构造函数）。**内部非命令函数**：`build_ocr_provider(tier)`（纯构造 det/rec/dict 模型，同步重活，调用方须 `spawn_blocking`）/ `ensure_ocr_provider(app)`（懒确保 provider 存在：None 则按当前 tier 重建并刷新 last_used，`ocr_init` 单飞锁 + 二次检查防并发惊群；recognize_region 入口取用） | `Config::load/save`、`build_provider`、`PaddleOcrProvider::new`、`translate::prompt::default_prompt_template` |
...
| `ocr_translate.rs` | `crop_region`（裁剪缓存帧+写临时 PNG+缓存进 `last_crop`+返回路径）/ `recognize_region`（从 `last_crop` 取图，经 `ensure_ocr_provider` 懒获取/重建 OCR Provider + `align` 前的原文清洗+缓存进 `last_ocr`+返回 OCR 行与整段原文）/ `translate_region`（从 `last_ocr` 取原文翻译+`align_lines` 配对+写历史+返回逐行译文/整段译文/Provider/耗时）；核心管线抽成纯函数 `run_ocr`+`run_translate`（不依赖 Tauri，便于 mock 测试）；图像编码 helper `encode_png`（PNG 写盘/入库复用）+ `read_last_crop_png`（取裁剪图编码入历史） | `OcrProvider::recognize`、`TranslationProvider::translate`、`HistoryStore::insert`、`ensure_ocr_provider` |
...
- `capabilities/default.json`：权限集（窗口创建/关闭/聚焦、webview、event、global-shortcut、clipboard、dialog）。**`windows` 白名单必须覆盖所有运行时窗口 label**——`main`（主窗，含 Onboarding/Home/Settings-via-router 等路由）、`capture`（选区窗）、`result`（结果窗）、`settings`（独立设置窗）、`history`（独立历史窗）。**漏列会导致该窗口无任何 Tauri 权限**：`invoke` 命令被拒、`listen` 事件收不到，页面能渲染但所有后端交互静默失败（2026-07-14 修复"设置页下载模型下不动"根因即 `settings`/`history` 缺失）。
...
> 已删（2026-06-29 迁移 Tauri 时清理）：`build-msi.ps1`（改用 Tauri NSIS/MSI bundler）、`mirror-models.ps1`、`verify-deps.ps1`。已删（2026-07-15 内存诊断完成后清理）：`mem-diag.ps1`（诊断压测脚本，诊断任务已完成）。
