...
1. 注册插件（single-instance / global-shortcut / clipboard-manager / dialog）
   - **single-instance**：第二实例聚焦主窗口；**诊断模式** argv 含 `--diag-ocr`[=`WxH`] 时触发 `diag_run_ocr_pipeline`（绕过 GUI，供 mem-diag.ps1 自动化压测），`parse_diag_bbox` 解析可选尺寸参数
...
进程树内存采样（诊断门禁，2026-07-15 新增）。Windows API 取主进程 + 所有子进程（WebView2）的 `WorkingSetSize`（工作集）+ `PrivateUsage`（私有字节）。**纯只读观测，无业务副作用**。关键 API：`GetProcessMemoryInfo`（取单进程内存，用 `PROCESS_MEMORY_COUNTERS_EX` 因有 `PrivateUsage` 字段）、`CreateToolhelp32Snapshot` + `Process32First/Next`（枚举进程树建 parent→children 映射，BFS 找全部后代）。公共 API：`snapshot_process_tree() -> ProcessTreeMemory`、`log_mem_diag(node, mem, extra)`（统一 `target="mem_diag"` 输出到 tracing 日志，便于 grep 过滤）。打点位置见各命令（capture/recognize/translate/history 前后）+ 前端经 `log_diag` 回传的事件点。**5 个单元测试**（`#[cfg(test)]`）：采样不 panic / 私有字节>0 / 工作集>=私有字节 / MB 换算边界 / spawn 子进程验证 BFS 枚举——CI 可跑，不依赖桌面会话。真实内存增长曲线验证由 `scripts/mem-diag.ps1` 驱动。**为什么单独建模块而非内联**：进程树遍历逻辑（snapshot + BFS + HANDLE RAII）有独立复杂度，且被 4 个命令文件复用，内联会重复。**为什么放 src-tauri 而非 core**：诊断是二进制层关注点，core 是纯逻辑库不依赖平台 API。
...
| `mem-diag.ps1` | 内存诊断压测（经 `snaptext.exe --diag-ocr` argv 触发后端管线 + 读 `[mem_diag]` 日志 + CSV 报告）。**绕过 GUI/鼠标**——直接让已运行实例在 Rust 内跑完整 capture→ocr→translate（见 `diag_run_ocr_pipeline`），比模拟鼠标可靠（选区窗 show 有异步时序）。**UTF-8 with BOM + CRLF**（PowerShell 5.1 硬要求，且不能有重复 BOM 否则 param 解析崩溃）。支持 `-Scenario full/random`（全屏验证线性增长 / 随机尺寸验证动态 shape 扩容），从日志尾部解析 `ocr_before/ocr_after` 的 `main_priv_mb`+`children_priv_mb`，输出判读指南 | 内存诊断 |
